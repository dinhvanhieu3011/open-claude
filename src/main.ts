...
async function startGlobalRecording() {
  ...
  // Start audio ducking to reduce system volume
  await audioDuckingManager.start();
...
ipcMain.handle('global-recording-complete', async (_event, audioData: ArrayBuffer, fileName?: string) => {
  try {
    console.time('[Transcribe] Total Time');
    console.log('[Global Recording] Transcribing audio...');

    // Stop audio ducking - restore system volume (non-blocking)
    ...
    console.timeEnd('[Transcribe] Total Time');

    // Send to main window for history
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('transcription-complete', finalKey);
...
// Audio ducking IPC handlers
ipcMain.handle('audio-ducking-start', async () => {
  await audioDuckingManager.start();
});

ipcMain.handle('audio-ducking-stop', async () => {
...
ipcMain.handle('warm-bearer-token', async () => {
  try {
    console.log('[Warm-up] Pre-warming bearer token...');
    await getBearerToken();
    console.log('[Warm-up] Bearer token cached');
  } catch (error) {
    console.error('[Warm-up] Failed to pre-warm bearer token:', error);
    // Don't throw - recording should continue even if warm-up fails
  }
});
